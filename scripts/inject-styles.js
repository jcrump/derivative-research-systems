 ...
 * inject-styles.js
 *
 * Finds blog HTML files that use external <link rel="stylesheet"> references
 * instead of inline styles, and replaces those <link> tags with the full
 * inline <style> block used by existing DRS posts.
 *
 * Run from repo root:
 *   node scripts/inject-styles.js
 *
 * Safe to run multiple times — skips files that already have inline styles.
 */
...
// ── The canonical DRS inline style block ─────────────────────────────────────
// Copied from existing working posts. Update here if the design ever changes.
...
// Also inject the Google Fonts preconnect + link if missing
...
if (!htmlFiles.length) {
  console.log('  ℹ No .html files found in blog/');
  process.exit(0);
}
...
for (const file of htmlFiles) {
  ...
  // Skip files that already have inline styles
  if (html.includes('<style>') || html.includes('<style ')) {
    console.log(`  ⟳ Skipping ${path.basename(file)} — already has inline styles`);
    skipped++;
    continue;
  }

  // Remove external stylesheet link tags pointing to styles.css / blog.css
  html = html.replace(/<link[^>]+rel=["']stylesheet["'][^>]*href=["'][^"']*(?:styles|blog)\.css[^"']*["'][^>]*\/?>/gi, '');

  // Remove any blank lines left behind by the removal
  html = html.replace(/\n(\s*\n){2,}/g, '\n\n');

  // Inject Google Fonts before </head> if not already present
...
console.log('  git commit -m "Fix CSS: inline styles into new posts"');
